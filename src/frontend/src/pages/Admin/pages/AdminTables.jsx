...
import { Plus, Search, Filter, Grid2x2Check, Calendar, Users, Clock, MoreVertical, Edit, Trash2, LayoutGrid, Armchair, XCircle, PersonStanding } from 'lucide-react';
import { cn } from "@/lib/utils";
import { format } from "date-fns";
import TableCard from '../Components/Tables/TableCard';
import AddTableModal from '../Components/Tables/AddTableModal';
import NewReservationModal from '../Components/Tables/NewReservationModal';
import WalkInModal from '../Components/Tables/WalkInModal';
import { LateArrivalTracker } from '../Components/Tables/LateArrivalTracker';
...
const AdminTables = () => {
    ...
    const filteredTables = tables.filter(table => {
        const matchesSearch = table.label && table.label.toLowerCase().includes(searchQuery.toLowerCase());
    ...
    const handleBookingSubmit = async (e) => {
        ...
        if (result.success) {
            ...
            toast.error("Failed to reserve: " + result.message);
        }
    };
    ...
    return (
        ...
        <div className="flex w-full h-full bg-background overflow-hidden relative">
            ...
            <div className="flex-1 min-w-0 flex flex-col h-full overflow-hidden border-r border-border/50">
                ...
                {/* Tables Grid */}
                <div className="flex-1 w-full overflow-auto min-h-0 custom-scrollbar relative">
                    <div className="px-8 pb-32 w-max min-w-full">
                        {isLoading ? (
                            ...
                            <div className="w-[1200px] flex flex-wrap justify-center gap-x-12 gap-y-16 pt-8 pb-12">
                            ...
                            <div className="space-y-6 pb-6">
                                        ...
                                        (() => {
                                            ...
                                            if (selectedTable.status === 'Available') {
                                                ...
                                                const hasUpcoming = reservations.some(r => {
                                                    ...
                                                    return resStart >= today && resStart < tomorrow;
                                                });
                                ...
                                {selectedTable.status === 'Occupied' ? (
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        {reservations && reservations.filter(r => r?.table?._id === selectedTable._id && r?.status === 'Pending').length > 0 ? (
                                            <div className="space-y-3">
                                                {reservations.filter(r => r?.table?._id === selectedTable._id && r?.status === 'Pending').map(res => (
                                                    <div key={res._id} className="rounded-md border p-4 space-y-3 bg-muted/20">
                                                        ...
                                                        <div className="grid grid-cols-2 gap-2 mt-2">
                                                            <Button onClick={() => checkInReservation(res._id).then(() => { getTables(); getReservations(); setSelectedTable({ ...selectedTable, status: 'Occupied' }); })} size="sm" className="w-full rounded-md text-xs">
                                                                Check-in
                                                            </Button>
