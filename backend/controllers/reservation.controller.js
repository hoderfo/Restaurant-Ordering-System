...
const findBestFitTable = async (guests, requestedStartTime, requestedEndTime, requireAvailableNow = false) => {
    ...
    query += ' ORDER BY capacity ASC, label ASC';
    ...
    for (const table of tables) {
        ...
        const reqDate = requestedStartTime.toISOString().split('T')[0];
        ...
        for (const res of dayResResult.rows) {
            // Reconstruct Date objects to compare
            const resStart = new Date(`${reqDate}T${res.start_time}Z`); // assuming UTC or local time depending on DB
            // Better to use requestedStartTime timezone. Let's just create dates using local time.
            const dbDateStr = res.date.toISOString().split('T')[0];
...
const createReservation = async (req, res) => {
    try {
        ...
        if (tableId) {
            ...
            const reqDate = bufferedStartTime.toISOString().split('T')[0];
            const dayResResult = await pool.query(
                "SELECT * FROM reservations WHERE table_id = $1 AND status IN ('pending', 'seated') AND date = $2",
            ...
            for (const r of dayResResult.rows) {
                const dbDateStr = r.date.toISOString().split('T')[0];
        ...
        const mappedReservation = {
            ...
            contact: r.phone,
            guests: r.party_size,
            startTime: new Date(`${r.date.toISOString().split('T')[0]}T${r.start_time}`)
        };
...
const cancelReservation = async (req, res) => {
    try {
        ...
        // Revert table to available if it was reserved
        await pool.query("UPDATE tables SET status = 'available' WHERE table_id = $1 AND status = 'reserved'", [reservation.table_id]);
